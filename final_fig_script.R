...
d<-dotplot(ego_upup_SIMPL, showCategory=15,orderBy = "x")+#, title = "Enriched terms among genes commonly \nup bound by SMC1A and H3K27ac")+
  theme(
    #plot.title = element_text(color = "black", size = 19),
    axis.title.x = element_text(color = "black", size = 8),
    axis.title.y = element_text(color = "black", size = 8),
...
ggsave("./final_fig/Figure3E_Enriched terms among genes commonly differentially up-bound by SMC1a and H3K27ac.png",d_a1,width = 120, height = 100,units ="mm" ,dpi = 300)
...
# Figure 4A-B
#######
mycolors<-c("firebrick4", "dark orange", "gold2", "olivedrab", "steelblue4", "orchid")
...
## GROUP A B C
groupa<-read.table("groupa.txt")
groupb<-read.table("groupb.txt")
groupc<-read.table("groupc.txt")
...
lupus_treat_sig<-na.omit(lupus_treatDATA_sig) #386
